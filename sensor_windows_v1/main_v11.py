...
num_of_board = 5        # Total number of patch used
num_of_tip = 1          # Fingertip flag
...
def csv_log_init():
    ...
    #if num_of_tip == 1:
    #    for i in range(num_of_board + num_of_tip, num_of_board + num_of_tip + 1):
    #        del_num = board_start_num + i
    #        if os.path.exists('LOG%s.csv' % (del_num)):
    #            os.remove('LOG%s.csv' % (del_num))
...
def read_sensor():
    ...
    #    if num_of_tip == 1:
    #        for j in range(board_start_num + num_of_board - num_of_tip,
    #                board_start_num + num_of_board):
    #            cmsg_array[j, 0].canWriteByte(cif_array[j, 0], (id_base|j), 2, 7, 0)
    #            for k in range(0, 8):
    #                cmsg_array[j, k].canRead(cif_array[j, k])
    ...
    #if num_of_tip == 1:
    #    for j in range(board_start_num + num_of_board,
    #            board_start_num + num_of_board + num_of_tip):
    #        for k in range(0, num_of_taxel / 2):
    #        mlx_buffer[j, k] = cmsg_array[board_start_num + j, k].data.c
