...
size = (num_of_board + num_of_tip) * num_of_taxel * data_length 
step = num_of_taxel * data_length
byte_array = bytearray(size)
byte_buffer = buffer(byte_array, 0, size)
shift_array = [None] * (num_of_board + num_of_tip) * num_of_taxel * num_of_axis

# Limitation initialisation and settings
difference = numpy.zeros((num_of_board + num_of_tip, num_of_taxel, num_of_axis))
tracking = numpy.zeros((num_of_board + num_of_tip, num_of_taxel, num_of_axis))
...
def record_baseline():
    ...
    #copyfile('./LOG1.csv', './LOG1.csv')
    copyfile('./LOG1.csv', './LOG2.csv')
    print('\nCompleted recording baseline\n')
...
def read_sensor_fingertip():

    # Receive can message
    try:
        for j in range(board_start_num + num_of_board, board_start_num + num_of_board + num_of_tip):
            cmsg_array[j, 0].canWriteByte(cif_array[j, 0], (id_base|j), 2, 7, 0)
            for k in range(0, num_of_taxel):
                cmsg_array[j, k].canRead(cif_array[j, k])
