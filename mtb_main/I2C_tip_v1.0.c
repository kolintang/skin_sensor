...
//data. The function returns "1" if success, otherwise "0".
//--------------------------------------------------------------------------------
...
unsigned char WriteViaI2C(unsigned char Channel,unsigned char DeviceAddress, const unsigned int RegisterStartAddress, const unsigned char NumberOfRegistersToWrite, unsigned int *DataBuffer, const unsigned int OffsetInBuffer)
{
    ...
    //Add the write bit to the device address
    DeviceAddressHeader = DeviceAddress<<1 | I2C_WR;
    //Split the address in two bytes  // K: no need for this for MLX, data already got split before sending to this function
...
///// MODIFIED VERSION
unsigned char SendViaI2C_MLX(unsigned char channel, unsigned char i2c_address, char *send_buffer, const unsigned char send_data_length, unsigned int *recieve_buffer_sda1, unsigned int *recieve_buffer_sda2, unsigned int *recieve_buffer_sda3, unsigned int *recieve_buffer_sda4, const unsigned char recieve_data_length)
{
    unsigned char r, AcknError;
    unsigned char DeviceAddressHeader;

    AcknError=1; //No error on initialization

    //Add the write bit to the device address
    DeviceAddressHeader = i2c_address << 1 | I2C_WR;
    ...
    if (!SendByteI2CMaster(channel,DeviceAddressHeader))
    {
        //Perform block write to send command address
            ...
            if (!SendByteI2CMaster(channel,send_buffer[0])) //for now it is only for 1 byte
            {
               //send the repeated start
                ...
                DeviceAddressHeader = i2c_address << 1 | I2C_RD;
                if (!SendByteI2CMaster(channel,DeviceAddressHeader))
                {
                    //Perform block read, but first,we need to know if we must send an ACKN or a NACK
                    if (recieve_data_length == 1) // the unit of length is in byte
                         {
                            //Do only one-byte read sending the NACK
                            ReceiveByteI2CMaster(channel,NACK);
                            recieve_buffer_sda1[0]=ReceivedByte[0];
                            recieve_buffer_sda2[0]=ReceivedByte[1];
                            recieve_buffer_sda3[0]=ReceivedByte[2];
                            recieve_buffer_sda4[0]=ReceivedByte[3];
                         } 
                    else
                        {
                       
                            for (r = 0; r < (recieve_data_length - 1); r++)
                            {
                            ReceiveByteI2CMaster(channel,ACK);
                            recieve_buffer_sda1[r]=ReceivedByte[0];
                            recieve_buffer_sda2[r]=ReceivedByte[1];
                            recieve_buffer_sda3[r]=ReceivedByte[2];
                            recieve_buffer_sda4[r]=ReceivedByte[3];
                            } 
                            //Do the last read sending the NACK
                        ReceiveByteI2CMaster(channel,NACK);
                        recieve_buffer_sda1[recieve_data_length - 1]=ReceivedByte[0];
                        recieve_buffer_sda2[recieve_data_length - 1]=ReceivedByte[1];
                        recieve_buffer_sda3[recieve_data_length - 1]=ReceivedByte[2];
                        recieve_buffer_sda4[recieve_data_length - 1]=ReceivedByte[3];
                        
                         } //end else
                    StopI2CMaster(channel);
                } //end if Device Address
                else // Device address;
                {
                    StopI2CMaster(channel);
                    AcknError=0;
            ...
            {
                StopI2CMaster(channel); // for command
                AcknError=0; 
            }
    }
    else //No acknowledgment was found therefore send the stop condition
    {
        StopI2CMaster(channel); // for device address
        AcknError=0;
    }
...
unsigned char SendViaI2C_MLX_finger(unsigned char channel, unsigned char i2c_address, char *send_buffer, const unsigned char send_data_length, unsigned int *recieve_buffer_sda1, unsigned int *recieve_buffer_sda2, const unsigned char recieve_data_length)
{
    unsigned char r, AcknError;
    unsigned char DeviceAddressHeader;
...
unsigned char SendViaI2C_MLX_onSdaX(unsigned char channel, unsigned char i2c_address, i2c_sda_num_t sdaNum ,char *send_buffer, const unsigned char send_data_length, unsigned int *recieve_buffer_sda, const unsigned char recieve_data_length)
{
    unsigned char r, AcknError;
    unsigned char DeviceAddressHeader;
    ...
    if (!SendByteI2CMaster_onSdaX(channel, sdaNum,DeviceAddressHeader))
    {
        //Perform block write to send command address
            ...
            if (!SendByteI2CMaster_onSdaX(channel, sdaNum,send_buffer[0])) //for now it is only for 1 byte
            {
               //send the repeated start
               StartI2CMaster_onSdaX(channel, sdaNum);
                    ...
                    if (recieve_data_length == 1) // the unit of length is in byte
                         {
                            //Do only one-byte read sending the NACK
                            ReceiveByteI2CMaster(channel,NACK);
                            recieve_buffer_sda[0]=ReceivedByte[0];
                    ...
                    else
                        {
                            ...
                            {
                            ReceiveByteI2CMaster(channel,ACK);
                            recieve_buffer_sda[r]=ReceivedByte[0];

                            } 
                            //Do the last read sending the NACK
                        ReceiveByteI2CMaster(channel,NACK);
                        recieve_buffer_sda[recieve_data_length - 1]=ReceivedByte[0];
            ...
            {
                StopI2CMaster_onSdaX(channel, sdaNum); // for command
                AcknError=0; 
            }
    }
    else //No acknowledgment was found therefore send the stop condition
    {
        StopI2CMaster_onSdaX(channel, sdaNum); // for device address
        AcknError=0;
    }
...
// This one is for sending 4 bytes. We have to send all commands before reading the status byte, so this function is separated from the 1byte version
// send_buffer must be an integer, otherwise it won't work
unsigned char SendCommandI2C_MLX(unsigned char channel, unsigned char i2c_address, int *send_buffer, unsigned int *recieve_buffer_sda1, unsigned int *recieve_buffer_sda2, unsigned int *recieve_buffer_sda3, unsigned int *recieve_buffer_sda4, const unsigned char recieve_data_length)
{
    unsigned char r, AcknError;
    unsigned char DeviceAddressHeader;
    ...
    if (!SendByteI2CMaster(channel,DeviceAddressHeader))
    {
        //Perform block write to send command address
            ...
            if (!SendByteI2CMaster(channel,send_buffer[0])) //first byte (command 1)
            {
                if (!SendByteI2CMaster(channel,send_buffer[1])) //second byte (command 2)
            {
                     if (!SendByteI2CMaster(channel,send_buffer[2])) //third byte (command 3)
            {
                          if (!SendByteI2CMaster(channel,send_buffer[3])) //fourth byte (command 4)
            {
               //send the repeated start
                StartI2CMaster(channel);
            ...
            {
                StopI2CMaster(channel); // command4
                AcknError=0; 
            }
                     } //end if command 3
                         else
            {
                StopI2CMaster(channel); // command3
                AcknError=0; 
            }
                }// end if command 2
                else
            {
                StopI2CMaster(channel); // command2
                AcknError=0; 
            }
            ...
            {
                StopI2CMaster(channel); // command1
                AcknError=0; 
            }
    }
    else //No acknowledgment was found therefore send the stop condition
...
/*
unsigned char SendViaI2C_MLX(unsigned char channel, unsigned char i2c_address, unsigned char *send_buffer, const unsigned char send_data_length, unsigned char *recieve_buffer_sda1, unsigned char *recieve_buffer_sda2, unsigned char *recieve_buffer_sda3, unsigned char *recieve_buffer_sda4, const unsigned char recieve_data_length)
{
    unsigned char r, AcknError;
    unsigned char DeviceAddressHeader;
    ...
    {
        //Perform block write
        for (r=0;r<send_data_length;r++)
        {
            SendByteI2CMaster(channel,send_buffer[r]);
        }
        //Stop transfer
        StopI2CMaster(channel);
    } 
    ...
    //Send device address
    if (!SendByteI2CMaster(channel,DeviceAddressHeader)) //DeviceAddressHeader = i2c_address << 1 | I2C_WR;
    {
        //Send the repeated start
        StartI2CMaster(channel);
        //Send device address again changing the Rd/Wr bit
...
*/
// send command 4bytes only to 1 SDA 

unsigned char SendCommandI2C_MLX_sdax(unsigned char channel, unsigned char i2c_address, i2c_sda_num_t sdaNum , int *send_buffer, unsigned int *recieve_buffer_sda, const unsigned char recieve_data_length)
{
    unsigned char r, AcknError;
    unsigned char DeviceAddressHeader;
    ...
    if (!SendByteI2CMaster(channel,DeviceAddressHeader))
    {
        //Perform block write to send command address
            ...
            if (!SendByteI2CMaster_onSdaX(channel, sdaNum,send_buffer[0])) //first byte (command 1)
            {
                if (!SendByteI2CMaster_onSdaX(channel, sdaNum,send_buffer[1])) //second byte (command 2)
            {
                     if (!SendByteI2CMaster_onSdaX(channel, sdaNum,send_buffer[2])) //third byte (command 3)
            {
                          if (!SendByteI2CMaster_onSdaX(channel, sdaNum,send_buffer[3])) //fourth byte (command 4)
            {
               //send the repeated start
                StartI2CMaster_onSdaX(channel, sdaNum);
            ...
            {
                StopI2CMaster_onSdaX(channel, sdaNum); // command4
                AcknError=0; 
            }
                     } //end if command 3
                         else
            {
                StopI2CMaster_onSdaX(channel, sdaNum); // command3
                AcknError=0; 
            }
                }// end if command 2
                else
            {
                StopI2CMaster_onSdaX(channel, sdaNum); // command2
                AcknError=0; 
            }
            ...
            {
                StopI2CMaster_onSdaX(channel, sdaNum); // command1
                AcknError=0; 
            }
    }
    else //No acknowledgment was found therefore send the stop condition
...
unsigned char ReadViaI2C(unsigned char Channel, unsigned char DeviceAddress, const unsigned int RegisterStartAddress, const unsigned char NumberOfRegistersToRead, unsigned int *DataBuffer1,unsigned int *DataBuffer2,unsigned int *DataBuffer3,unsigned int *DataBuffer4, const unsigned int OffsetInBuffer)
{
    ...
    AcknError=1; //No error on initialization // so 1 mean NO
...
//Function that reads from  via the I2C port. It sends first the device
//address including the write bit, then the register address and finally reads data
//back. The function returns "1" if success, otherwise "0". If an error occurs,
//Then the stop condition is sent.
//--------------------------------------------------------------------------------
